      ...
      <div className="flex flex-col gap-3 mb-4 sm:mb-8 bg-white dark:bg-[#121212] p-3 sm:p-4 rounded-xl border border-slate-200 dark:border-[#2A2A2A] shadow-sm">
        <div className="flex flex-wrap items-center gap-2 sm:gap-4 overflow-x-auto">
          {[
            ...
            >
              <div className={`w-6 h-6 rounded-full flex flex-shrink-0 items-center justify-center text-sm font-bold transition-all duration-300 ${
                step === s.id ? 'bg-[#FF3366] text-white' : step > s.id ? 'bg-emerald-500/10 sm:bg-emerald-500/20 text-emerald-500 dark:text-emerald-400' : 'bg-slate-100 dark:bg-zinc-800 text-slate-400 dark:text-zinc-500'
              }`}>
                {s.id}
              </div>
      ...
      <div className="bg-white dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#2A2A2A] p-4 sm:p-6 shadow-sm sm:shadow-xl transition-all duration-500 ease-in-out relative min-h-[500px]">
        {isLoading && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/80 dark:bg-[#121212]/80 backdrop-blur-sm rounded-xl animate-in fade-in duration-300">
             ...
             <div className="mt-4 font-bold text-slate-700 dark:text-zinc-300 animate-pulse text-sm uppercase tracking-widest">Processando IA...</div>
          </div>
        )}
