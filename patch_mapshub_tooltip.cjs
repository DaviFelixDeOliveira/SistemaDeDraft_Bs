...
code = code.replace(
  /<span className=\{cn\(\s*"text-\[10px\] uppercase font-bold tracking-wider px-2 py-0\.5 rounded-full border",\s*map\.terrain === 'Aberto' \? 'bg-blue-500\/10 text-blue-600 border-blue-500\/20' :\s*map\.terrain === 'Fechado' \? 'bg-orange-500\/10 text-orange-600 border-orange-500\/20' :\s*'bg-emerald-500\/10 text-emerald-600 border-emerald-500\/20'\s*\)\}>\s*\{map\.terrain\}\s*<\/span>/,
  `<div className="relative group/tooltip">
                        <span className={cn(
                          "text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border cursor-help flex items-center gap-1",
                        ...
                        <div className="absolute bottom-full right-0 mb-2 w-48 p-2 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-[10px] sm:text-xs rounded-lg shadow-xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all z-20 font-medium pointer-events-none">
